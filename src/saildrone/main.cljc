(ns saildrone.main
  "Kotodama WASM entrypoint for the Saildrone clean-room actor (L4) — Clojure port.
  ...
  facts (`saildrone.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "saildrone")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"saildrone.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "saildrone-compat" :tier tier :entities entities} 200])
...
(defn start! [] :saildrone-compat/ready)
